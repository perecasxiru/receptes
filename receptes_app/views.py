...
from .services import get_all_rows, get_worksheet
...
def get_recipe_list(request):
    ...
    #                           rec.image.url if rec.image else None, "|".join([t.name for t in rec.tags.all()]), "|".join([t.name for t in rec.tools.all()])])
